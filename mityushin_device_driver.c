...
#define AUTHOR "DMITRY MITYUSHIN"
#define DESCRIPTION "DEVICE FOR READ AND WRITE DATA"
#define LICENSE "GPL"
...
/*number of empty slots in the buffer need at least 1 empty slot to write need at least 1 filled slot to read*/
...
module_param(device_name, charp,
0000);

/*getting string_char_count*/
module_param(string_char_count,
int, 0000);
...
int init_module() {
    ...
    if ((register_return_value = misc_register(&my_device))) {
        /*misc_register() returns 0: success -ve: failure*/
        printk(KERN_ERR
    ...
    /*allocating memory for the buffer. 2d array*/ int _allocated = 0;
...
static ssize_t my_read(struct file *_file, char *user_buffer, size_t number_of_chars_to_be_read, loff_t *offset) {
    ...
    /*acquiring lock on read_op_mutex performing down()
     * for the buffer instance and copying it into the user
     * buffer performing up() on the buffer instance and
     * releasing lock on read_op_mutex*/
    down_interruptible(&read_op_mutex);
    down_interruptible(&full);
    int timer = 0;
    while (buffer_size == buffer_empty_slots && timer < 1000000) { timer++; }
    read_index %= buffer_size;
    for (user_buffer_index = 0; user_buffer_index < number_of_chars_to_be_read; user_buffer_index++) {
        if (buffer_empty_slots >= buffer_size) { break; }
...
static ssize_t my_write(struct file *_file, const char *user_buffer, size_t number_of_chars_to_write, loff_t *offset) {
    int user_buffer_index = 0;
    int i = 0;
    /*acquiring lock on write_op_mutex performing down() for the buffer instance
     * and writing characters to the buffer performing up() on the buffer instance
     * and release lock on the buffer instance*/
...
void cleanup_module() {
    /*freeing memory*/
    int _iter;
    for (_iter = 0; _iter < buffer_size; _iter++) { kfree(buffer[_iter]); }
